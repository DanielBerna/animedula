  ...
  return (
    <article className="poster-card enter-up group">
      <Link href={`/anime/${slug || ''}`} className="block">
        <div className="relative overflow-hidden poster-img-wrap">
          {image ? (
            <PosterImage src={image} alt={title} fill className="poster-img object-cover" />
          ) : (
            <div className="poster-img bg-surface-4 flex items-center justify-center">
              <IconOrb name="anime" variant="anime" size="md" />
            </div>
          )}
