...
export default function ProductCard({ producto, variant = 'default' }: Props) {
  const url = buildGoUrl(producto.partner, {
    query: producto.query,
    dest: producto.asin,
    anime: producto.anime,
    malId: producto.malId,
  })
  const cta = producto.partner === 'mercadolibre' ? 'Buscar en Mercado Libre' : 'Ver en Amazon'
  const ratio = variant === 'manga' ? 'aspect-[3/4]' : 'aspect-[4/3]'

  return (
    <article className="product-card enter-up">
      <a href={url} target="_blank" rel="noopener noreferrer sponsored" className="product-card-link group">
