...
export async function listUgcReviewsByStatus(
  status: 'pending' | 'published' | 'rejected',
  limit = 50,
): Promise<UgcReviewItem[]> {
  if (!isSupabaseAuthConfigured()) return []
...
export async function updateUgcReview(
  reviewId: number,
  patch: { comment?: string; rating_global?: number; is_spoiler?: boolean },
) {
  if (!isSupabaseConfigured()) throw new Error('Supabase no configurado')
  const admin = getSupabaseAdmin()
  const { error } = await admin.from('user_reviews').update(patch).eq('id', reviewId)
  if (error) throw error
}
